...
// export function deleteEquivalentRow(table, output){
//   console.log("test");
//   for(let i=0; i<output.length; i++){
//     if(output[i].length != 0){
...
//   let output = findAndReplaceFullyEquivalentRows(input);
//   input = deleteEquivalentRow(test,output);
...
//   output = findAndReplaceFullyEquivalentRows(stateTable);
//   stateTable = deleteEquivalentRow(stateTable,output);
...
function solve(input) {
  ...
  replaceEquivalentRow(stateTable,EqState);
  // stateTable = reduceStateTable(stateTable,equivalentStates);
  iterations.push(_.cloneDeep(stateTable));
  return iterations;
}
